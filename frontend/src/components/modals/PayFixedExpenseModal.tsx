...
export default function PayFixedExpenseModal({ fixedExpense, onClose, onSuccess }: PayFixedExpenseModalProps) {
  ...
  useEffect(() => {
    Promise.all([api.getSources(), api.getCategories()]).then(([s, cats]) => {
      setSources(s)
      setCategories(cats)
      // Only set defaults if not prefilled from fixed expense
      if (!fixedExpense.source_id) {
        const def = s.find(x => x.is_default) ?? s[0]
        if (def) setSourceId(def.id)
      }
    })
  }, [fixedExpense.source_id])
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      // Advance the fixed expense lifecycle
      await api.payFixedExpense(fixedExpense.id)
      onSuccess()
    } catch (err: unknown) {
    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
      <div className="bg-white dark:bg-gray-900 rounded-xl shadow-xl w-full max-w-md max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-700 sticky top-0 bg-white dark:bg-gray-900">
          <div>
            <h2 className="text-lg font-semibold text-gray-900 dark:text-gray-100">Pay Fixed Expense</h2>
            <p className="text-xs text-gray-400 dark:text-gray-500 mt-0.5">Record a payment for "{fixedExpense.title}"</p>
          </div>
